...
class WalletViewController: BaseViewController {
    ...
    lazy var stepView: ProductHeadView = {
        ...
        attributedString.append(NSAttributedString(string: "4", attributes: numberAttributes))
    ...
    override func viewDidLoad() {
        ...
        footerView.nextBlock = { [weak self] in
            ...
            Task {
                await self.saveWalletInfo(with: parameters)
            }
        }
...
extension WalletViewController {
    ...
    private func getWalletInfo() async {
        do {
            ...
            let model = try await viewMdoel.walletApi(parameters: parameters)
    ...
    private func saveWalletInfo(with parameters: [String: String]) async {
        do {
            let model = try await viewMdoel.saveWalletApi(parameters: parameters)
            let peaceent = model.peaceent ?? ""
            if peaceent == "0" || peaceent == "00" {
